...
| `make tlg-twitch-gic` | Fit real Twitch networks with the TLG and rank families by spectral GIC. TLG: d̂ by GIC over candidate depths (degenerate depths excluded by a density-blow-up probe), σ/α by logistic regression (`mle`) or min-GIC Nelder–Mead search (`gic`), GIC by edge-gated monitored growth seeded at the real density (grow to ≈real edge count, then early-stop on min GIC). Baselines ER/BA/WS/KR/GRG/SBM by closed-form. Per-graph table: GIC + rank + both GIC terms (2·KL, 2·n_params) + edges/density/clustering/assortativity. `tlg-twitch-gic-all` runs all six countries. |
| `make tlg-twitter-gic` | Same methodology on the 973 Twitter SNAP **ego networks** (small, dense, highly clustered): sample ego nets in a size window, score each, and **aggregate** the family rankings (mean GIC rank, mean KL, win counts) across nets. Reuses the Twitch TLG-GIC core. |
