...
tlg-param-behavior:  ## TLG (sigma,alpha) behavior sweep for n={50,200,500}: centrality, clustering, scale-free / power-law metrics + heatmaps (reproducible, cached)
	$(UV) run python scripts/experiments/run_tlg_param_behavior.py

tlg-param-behavior-quick:  ## Smoke run of the TLG param-behavior sweep (n=50, tiny grid, ~30s)
	LG_TLGB_QUICK=1 \
		$(UV) run python scripts/experiments/run_tlg_param_behavior.py
